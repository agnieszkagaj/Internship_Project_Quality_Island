...
public class UzytkownicyPageTest extends TestBase {
    ...
    @Test(priority = 10, enabled = true, description = "Weryfikacja czy po  zmianie roli na 'Menadzer Tresci AnonimKlient' zmieni sie ona rowniez w zakladce w tabeli 'Uzytkownicy'")
    public void weryfikacjaZmianyRoliNaMenadzerTresciAnonimKlient(){
        ...
        edytujUzytkownikaPage.edytujRoleUzytkownikaNaMenadzerTresciAnonimKlient();
        edytujUzytkownikaPage.nacisnijPrzyciskZaktualizujKontoUzytkownika();
        uzytkownicyPage.cofnijSieDoStronyUzytkownicy();
        uzytkownicyPage.nacisnijPrzyciskLupy();
        uzytkownicyPage.wpiszNazweDoPolaWpiszByWyszukacNazwe();

        Assert.assertTrue(uzytkownicyPage.sprawdzCzyUzytkownikPosiadaRoleMenadzerTresciAnonimKlient(), "Zmiana roli nie powiodla sie.");
    }
