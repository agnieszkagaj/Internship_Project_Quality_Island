...
import static helpers.Utils.generujLiczbeOd1Do100000JakoString;
 ...
 public class UzytkownicyPage {
    ...
    @FindBy(xpath = "//td[contains(., 'Menedżer treści AnonimKlient')]")
    private WebElement rolaMenadzerTresciAnonimKlient;
    ...
    public void cofnijSieDoStronyUzytkownicy(){
     ...
     public boolean sprawdzCzyUzytkownikPosiadaRoleMenadzerTresciAnonimKlient(){
        boolean status = false;

        if(wait.waitForVisibility(rolaMenadzerTresciAnonimKlient).isDisplayed()){
            status = true;
